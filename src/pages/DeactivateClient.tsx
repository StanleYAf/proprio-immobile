...
export default function DeactivateClient() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.codigo) { toast.error('Informe o código do cliente'); return; }
    setLoading(true);
    const res = await sendToWebhook({
      tipo: 'cliente', acao: 'desativar',
  ...
  return (
    <AppLayout title="Desativar Cliente">
      <form onSubmit={handleSubmit} className="space-y-3">
        <FormField label="Código do Cliente" name="codigo" value={form.codigo} onChange={update} required placeholder="Ex: CLI-0015" />
        <FormField label="Motivo" name="motivo" value={form.motivo} onChange={update} multiline placeholder="Motivo da desativação" />
